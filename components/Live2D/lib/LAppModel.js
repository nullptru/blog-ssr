...
LAppModel.prototype.load = function (gl, modelDefine, callback) {
  ...
  const path = thisRef.modelHomeDir + thisRef.modelSetting.getModelFile();
  thisRef.loadModelData(path, (model) => {
    for (let i = 0; i < thisRef.modelSetting.getTextureNum(); i++) {
      const texPaths = thisRef.modelHomeDir +
                    thisRef.modelSetting.getTextureFile(i);

      thisRef.loadTexture(gl, i, texPaths, () => {
        if (thisRef.isTexLoaded) {
          if (thisRef.modelSetting.getExpressionNum() > 0) {
            ...
            for (var j = 0; j < thisRef.modelSetting.getExpressionNum(); j++) {
              ...
              const expFilePath = thisRef.modelHomeDir +
